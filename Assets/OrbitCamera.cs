        ...
        void LateUpdate()
        {
            ...
            if (target)
            {
                if (Input.GetMouseButton(0) || Input.GetMouseButton(1))
                {
                    ...
                    currentPos = Vector3.Lerp(currentPos, target.transform.position, Time.deltaTime*10.0f);
                    var position = rotation * new Vector3(0.0f, 0.0f, -distance) + currentPos;
                ...
                {
                    currentPos = Vector3.Lerp(currentPos, target.transform.position, Time.deltaTime*10.0f);
                    var position = transform.rotation * new Vector3(0.0f, 0.0f, -distance) + currentPos;
                    transform.position = position;
                }
            }
